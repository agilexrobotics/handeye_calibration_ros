#!/usr/bin/env python3
# -*-coding:utf8-*-
# agx_arm 机械臂手眼标定版:订阅 /feedback/tcp_pose (geometry_msgs/msg/PoseStamped)
...
class AgxArmCalibrationNode(Node):
    def __init__(self):
        ...
        print(f"agx_arm_topic: {self.agx_arm_topic}")
    ...
    def process_handeye(self):
        R_gripper2base = [data.rotation_matrix for data in self.agx_arm_poses]
        t_gripper2base = [data.translation_matrix for data in self.agx_arm_poses]
    ...
    def get_poses(self):
        ...
        while True:
            ...
            try:
                ...
                if user_input == '':
                    count += 1
                    marker_pose_raw, agx_arm_pose_raw = self.subscribe_message()
                    agx_arm_pose = PoseMartix(agx_arm_pose_raw.pose, True, self.mode)
                    marker_pose = PoseMartix(marker_pose_raw.pose, False, self.mode)
                    print("---")
                    print(f"agx_arm: {agx_arm_pose.position_list}")
                    print(f"marker: {marker_pose.position_list}")
                    self.agx_arm_poses.append(agx_arm_pose)
                    self.marker_poses.append(marker_pose)
                elif count > 1 and user_input == 'd':
                    count -= 1
                    self.agx_arm_poses.pop()
    ...
    def subscribe_message(self):
        ...
        sys.stdout.write("wait agx_arm data... ")
        sys.stdout.flush()
        agx_arm_pose_raw = self.subcribe_one_message(self.agx_arm_topic, PoseStamped)
        sys.stdout.write("[ok]\n")

        return marker_pose_raw, agx_arm_pose_raw
...
def main(args=None):
    ...
    calibration_node = AgxArmCalibrationNode()
